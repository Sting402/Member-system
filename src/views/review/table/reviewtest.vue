...
export default {
  ...
  async mounted() { // changed from onMounted() to mounted()
    console.log("123");
  try {
    const response = await axios.post(
      "http://220.133.235.3:5040/api/Register/AdminListJoinOrder");
      ...
      >
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'status'" class="block w-full">
            <span
              class="inline-block px-3 min-w-[90px] text-center mx-auto py-1 rounded-[10px] bg-opacity-20"
              :class="`${
                props.row.status === '全部出貨'
