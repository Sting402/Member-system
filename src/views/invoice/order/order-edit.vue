<template>
  <div>
    <div class="lg:flex justify-between flex-wrap items-center">
      ...
      <div class="flex lg:justify-end items-center flex-wrap">
        ...
        <button
          type="button"
          @click="print"
          class="invocie-btn inline-flex btn btn-sm whitespace-nowrap space-x-1 cursor-pointer bg-white dark:bg-slate-800 dark:text-slate-300 btn-md h-min text-sm font-normal text-success-600 rtl:space-x-reverse"
        >
          <span class="text-lg"><Icon icon="heroicons:truck" /></span>
          <span>訂單出貨</span>
        </button>
        <button
          class="invocie-btn inline-flex btn btn-sm whitespace-nowrap space-x-1 cursor-pointer bg-white dark:bg-slate-800 dark:text-slate-300 btn-md h-min text-sm font-normal text-danger-600 rtl:space-x-reverse"
        >
          <span class="text-lg"><Icon icon="heroicons:trash" /></span>
          <span>取消訂單</span>
        </button>
    ...
    <Card bodyClass="p-2">
      ...
      <div class="bg-slate-100 dark:bg-slate-700 rounded-t-lg px-4 pt-4 pb-1 grid grid-cols-2 md:grid-cols-6 gap-4">
        <div class="mr-3 mb-3 space-y-2">
          <div class="text-xs font-medium text-slate-600 dark:text-slate-300">
            訂單編號
          </div>
          <div class="text-s font-bold text-slate-600 dark:text-slate-300">
            #2212250245
          </div>
        </div>
        <!-- end single -->
        <div class="mr-3 mb-3 space-y-2">
          ...
          <div class="text-s font-bold text-danger-500">待出貨</div>
        </div>
        <!-- end single -->
        <div class="mr-3 mb-3 space-y-2">
          <div class="text-xs font-medium text-slate-600 dark:text-slate-300">
      ...
      <div class="bg-slate-100 dark:bg-slate-700 rounded-b-lg px-4 pt-4 pb-1 grid grid-cols-2 md:grid-cols-6 gap-4">
        <div class="mr-3 mb-3 space-y-2">
          <div class="text-xs font-medium text-slate-600 dark:text-slate-300">
            訂購人編號
          </div>
      ...
      <div class="grid grid-cols-12 gap-5">
        <div class="md:col-span-3 col-span-12">
          <div class="max-w-[1000px] mx-auto shadow-base dark:shadow-none my-8 rounded-md overflow-x-auto">
            <Card>
              <ul class="relative ltr:pl-2 rtl:pr-2">
                <li v-for="(item, i) in trackingOrder.slice(4, 8)" :key="i" :class="
                          item.status === 'ok'
                            ? 'before:opacity-100'
                            : ' before:opacity-50'
                        "
...
import { message, TodoList, trackingOrder } from "../../../constant/main-data"; 
export default {
  components: {
    VueSelect,
    Card,
