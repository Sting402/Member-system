<template>
  <div>
    <div class="filegroup">
      <label :for="name">
        <input type="file" @change="onChange" class="bg-red-400 w-full hidden" :id="name" :name="name"
          :multiple="multiple" :preview="preview" :placeholder="placeholder" :label="label"
          accept="image/png, image/jpeg" />
        <div class="w-full h-[40px] file-control flex items-center" :class="`  ${classInput}`">
          <span v-if="!multiple" class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
            <span v-if="selectedFile?.name" :class="
          ...
          <span v-if="multiple" class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
            <span v-if="multipleurls.length > 0" :class="
              filenametag ? ' badge-title' : 'text-slate-900 dark:text-white'
            ">{{
  ...
  : ""
        ...
        <div class="flex flex-wrap space-x-5 rtl:space-x-reverse" v-if="multipleurls.length > 0 && multiple && preview">
          <div v-for="(url, index) in multipleurls" :key="index" class="xl:w-1/5 md:w-1/3 w-1/2 rounded mt-6">
            <img :src="url" class="object-cover w-full h-full rounded" :alt="selectedFile?.name" />
          </div>
        </div>
      </label>
    </div>
...
export default {
  ...
  methods: {
    onChange(e) {
      ...
      console.log(form);
      axios.post('http://220.133.235.3:5040/api/FileUpload/RegisterFileUpload', form)
